...
把"一组素材图"变成亚马逊后台的**高级 A+(Premium A+)草稿**。识别/选型/文案由 agent(Claude Code 或 Codex)完成,**创建执行由中心服务**(装了紫鸟的公共电脑)通过 HTTP 接口完成。本目录只含**技能/指令 + 文档**,不含服务端代码。
...
- `codex/` —— Codex 版(`SKILL.md`(可当 skill 装)+ `AGENTS.md`(可并入 AGENTS)+ 同一套 reference)
- `.env.example` —— 配置模板
...
## 安装:Codex
两种方式任选:
- **当 skill 装(推荐)**:`codex/` 已含 `SKILL.md`,整个 `codex/` 放进 `~/.agents/skills/ziniao-premium-aplus/`(或项目 `.agents/skills/`,Codex 从 cwd 向上扫自动发现);配 `APLUS_BASE_URL` / `APLUS_API_KEY`。
