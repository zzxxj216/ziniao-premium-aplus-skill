...
description: >
  ...
  运营给一个素材文件夹/图片,你(Codex)识别图片、选合适的模块类型、
  生成所需文案,再调用中心服务(紫鸟)把高级 A+ 草稿建出来。
  不要用于:标准 A+、改价/库存、直接改 listing。
---
...
> 本文件是 `codex/` 作为「Codex skill」安装时的入口;与同目录 `AGENTS.md` 内容一致(后者用于并入 `~/.codex/AGENTS.md` 的方式)。

## 何时用
用户要给亚马逊商品做**高级 A+(Premium A+)**(给了一组图/文案)时用本技能。
